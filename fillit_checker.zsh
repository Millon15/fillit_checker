...
if [[ $1 -ne "-v" && $1 -ne "-i" ]] || [[ $2 == "" || $3 == "" ]] || ! $(ls $2/Makefile $3/Makefile 1>/dev/null); then
...
echo "${MAG}If your computer will be close to blow himself just press ^C and type in your shell the command below"
echo ">>> kill -9 \$(pgrep fillit)"
# echo "${CYAN}Press 'k' if you want to kill all 'fillit' processes immediately!"
printf $BLUE
...
if [[ $1 == "-v" ]]; then
	...
	for i in {1..${LIMITS[1]}}; do
		for j in {${LIMITS[2]}..${LIMITS[3]}}; do
			f="${i}_${j}"
			if [[ "$(head $TDIR/${FST}_${f}.out)" && "$(head $TDIR/${SCD}_${f}.out)" && "$(diff $TDIR/${FST}_${f}.out $TDIR/${SCD}_${f}.out)" ]]; then
